...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            #print(label)
            box = label.box
            box_corners =  tools.compute_box_corners(box.center_x, box.center_y, box.width, box.length, 0)
            ## step 2 : loop over all detected objects
            for det in detections:
                ## step 3 : extract the four corners of the current detection
                id, x, y, z, h, w, l, yaw = det
                det_corners = tools.compute_box_corners(x, y, w, l, 0) 
                ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                dist = np.array([box.center_x, box.center_y, box.center_z]) - np.array([x, y, z])
                
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                label_poly = Polygon(box_corners)
...
def compute_performance_stats(det_performance_all):
    ...
    false_negatives = 0
    false_positives = 0
    for data in pos_negs:
        all_positives = all_positives + data[0]
        true_positives = true_positives + data[1]
        false_negatives = false_negatives + data[2]
        false_positives = false_positives + data[3]
    print(all_positives)
